...
const BOTTOM = -5
const MID = -6
const labels = ["", "", "LEFT", "RIGHT", "TOP", "BOTTOM", "MID"]
// 時間をdtだけ進める。
...
// いろんな衝突のケースを考える必要がある。
// 壁との衝突と、粒子同士の衝突を分ける。
function wall(dt, x, y, vx, vy, last, kind)
{
    var collide = NOCOLL
    var t_coll = dt
    // 左の壁
    ...
    if (kind){
        ...
        // 下の壁
        if ( last != MID ){
            let t_mid = (height / 2-y) / vy
            if ( (0 <= t_mid ) && (t_mid < t_coll ) ){
                t_coll = t_mid
                collide = MID
            }
        }
...
function progress(dt, last)
{
    ...
    for(let i=0; i<N; i++){
        ...
        let col = wall(dt, x[i], y[i], vx[i], vy[i], lastwall, kind[i])
        let tc = col[0]
        let obj = col[1]
        if ( (0 <= tc)&&(tc < t_coll) ){
            obj1 = obj
    ...
    if ( obj1 != NOCOLL ){
        if ( ( obj1 == BOTTOM ) || ( obj1 == TOP ) || (obj1 == MID)){
            vy[obj2] = -vy[obj2]
        }
