...
function simulate() {
  ...
  if (mdlj.steps % mdlj.lsteps == 0) {
    ...
    output.innerHTML +=
      (mdlj.dt * mdlj.steps).toFixed(2) +
      "\t" +
      (mdlj.ep * 0.001).toPrecision(6);
    output.innerHTML +=
      "\t" + T.toPrecision(6) + "\t" + pressure.toPrecision(6) + "\n";
  }
  ...
  if (mdlj.steps % mdlj.Nstep == 0) {
    ...
    output.innerHTML +=
      "               Sigma [pm] " + mdlj.sig.toFixed(0) + "\n";
    output.innerHTML +=
      "           Epsilon/kB [K] " + mdlj.eps.toFixed(0) + "\n";
    output.innerHTML +=
      "         Density [g /cm3] " + mdlj.densr.toPrecision(6) + "\n";
    output.innerHTML +=
      "          Temperature [K] " + mdlj.tempr.toFixed(1) + "\n";
    output.innerHTML +=
      "Potential Energy [kJ/mol] " + (ep * 0.001).toPrecision(6) + "\n";
    output.innerHTML +=
      "           Pressure [MPa] " + pressure.toPrecision(6) + "\n\n";
  }
  ...
  } else if (status != 2) window.setTimeout(simulate, 1); // runs as fast as pos
}
...
function changeS() {
  ...
  startButton.disabled =
    isNaN(eps) || isNaN(sig) || isNaN(Temperature) || isNaN(Density);
}
