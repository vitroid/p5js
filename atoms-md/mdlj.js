...
class MDLJ {
    ...
    force_LJ(x, y, z, x2, y2, z2){
        ...
        let d6 = d2*d2*d2
        let d12 = d6*d6
        let f = 4*this.eps*Rgas*(-12*this.sig**12/d2**6 + 6*this.sig**6/d2**3)/d2
        let virial = f*d2
    ...
    energy_LJ(x, y, z, x2, y2, z2){
        ...
        let d6 = d2*d2*d2
        let d12 = d6*d6
        let epot = 4*this.eps*Rgas*(this.sig**12/d2**6 - this.sig**6/d2**3)
        let f = 4*this.eps*Rgas*(-12*this.sig**12/d2**6 + 6*this.sig**6/d2**3)/d2
        // 圧力の計算に必要
        let virial = f*d2
        return [epot, virial]
    ...
    progress(){
        ...
        for(let i=0; i<this.N; i++){
            for(let j=0; j<i; j++){
                let f = this.force_LJ(this.x[i],this.y[i],this.z[i],this.x[j],this.y[j],this.z[j])
                fx[i] += f[0]
                fy[i] += f[1]
                fz[i] += f[2]
                fx[j] -= f[0]
    ...
    statistics()
    {
        ...
        for(let i=0; i<this.N; i++){
            for(let j=0; j<i; j++){
                let ev = this.energy_LJ(this.x[i],this.y[i],this.z[i],this.x[j],this.y[j],this.z[j])
                this.ep += ev[0]
                this.vr += ev[1]
            }
        }
        ...
        this.ek = 0
        for(let i=0; i<this.N; i++){
            this.ek += this.mass*(this.vx[i]**2 + this.vy[i]**2 + this.vz[i]**2)/2
        }
        this.ek /= this.N
